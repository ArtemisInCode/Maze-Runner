...
void update_map (uint16_t collectables[], uint16_t map[], uint16_t player_X, uint16_t player_Y)
{
    ...
    for (uint8_t i = (0 + player_X); i<(5 + player_X); i++) { // Moves accross 5 columns of the ledmatrix and bitmap to be displayed
        if (i-2 >= 0 && i-2 <= 19 ) { // Diplays bitmap if within bounds of bitmap array
            if (player_Y <= 3) { // If player futher left than Y=3 shift direction is reversed so that top right of the map is X=0 Y=0 
                col = ((map)[i-2] << (3 - player_Y)) | ((collectables)[i-2] << (3 - player_Y)); // Sets value of column to be displayed 
            } else {
                col = ((map)[i-2] >> (player_Y - 3)) | ((collectables)[i-2] >> (player_Y - 3)); // Sets value of column to be displayed 
            }
            ledmat_display_column (col, i-player_X); // Displays column of bitmap
        } else {
